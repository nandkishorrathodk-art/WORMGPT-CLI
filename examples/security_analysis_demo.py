...
def demo_security_analyzer():
    ...
    if not slither_check["data"]["available"]:
        print(
            "   [!] Slither not installed. Install with: pip install slither-analyzer"
        )
    ...
    if analysis_result["success"]:
        ...
        if data["vulnerabilities"]:
            critical_vulns = [
                v for v in data["vulnerabilities"] if v["impact"] in ["High", "Medium"]
            ]

            if critical_vulns:
                poc_plan_result = drone.execute(
                    "generate_poc_plan", {"vulnerability": critical_vulns[0]}
                )
