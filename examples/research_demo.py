...
def demo_web_search():
    ...
    if result["success"]:
        console.print(
            f"[green]SUCCESS[/green] Found {result['data']['count']} results\n"
        )
...
def demo_web_fetch():
    ...
    if result["success"]:
        console.print(
            f"[green]SUCCESS[/green] Fetched {result['data']['content_length']} characters\n"
        )
...
def demo_research_drone():
    ...
    result = research_drone.execute(
        "search_and_summarize", {"query": query, "max_results": 3, "fetch_top_n": 1}
    )

    if result["success"]:
        data = result["data"]
        console.print(
            f"[green]SUCCESS[/green] Found {data['total_results']} results, fetched {data['fetched_count']} articles\n"
        )
