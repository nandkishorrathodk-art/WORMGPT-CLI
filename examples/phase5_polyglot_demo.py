...
from wormgpt_hive.shared.config import OPENROUTER_API_KEY, OPENROUTER_BASE_URL, QUEEN_MODEL
...
def demo_polyglot_code_execution():
    ...
    result = interpreter.execute(
        language="python",
        code='print("Hello from Python! 2+2 =", 2+2)'
    )
    if result["success"]:
    ...
    result = interpreter.execute(
        language="node",
        code='console.log("Hello from Node.js! Array:", [1,2,3]);'
    )
    if result["success"]:
        print(f"   Output: {result['data']['stdout'].strip()}")
    else:
