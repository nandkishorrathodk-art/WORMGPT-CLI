...
class ShellExecutorTool(BaseTool):
    ...
    def execute(
        ...
        try:
            ...
            return self._error_response(
                f"Failed to execute command: {str(e)}", f"Command: {command}"
            )
    ...
    def execute_script(
        self, script_content: str, script_type: str = "bash", cwd: Optional[str] = None
    ) -> Dict[str, Any]:
        try:
            interpreters = {
            ...
            return self._error_response(
                f"Failed to execute script: {str(e)}", f"Script type: {script_type}"
            )
