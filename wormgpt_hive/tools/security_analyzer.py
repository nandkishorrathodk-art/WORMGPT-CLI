...
class SecurityAnalyzerTool(BaseTool):
    ...
    def analyze_contract(self, contract_code: str, contract_name: str = "Contract") -> Dict[str, Any]:
        try:
            ...
            try:
                result = self.analyze_file(file_path=temp_file, contract_name=contract_name)
                return result
            finally:
                try:
                    os.unlink(temp_file)
    ...
    def get_vulnerability_report(self, vulnerabilities: List[Dict[str, Any]], format: str = "markdown") -> Dict[str, Any]:
        try:
            if format == "markdown":
    ...
    def _parse_slither_output(self, slither_output: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        vulnerabilities.sort(
            key=lambda x: (
                {"High": 0, "Medium": 1, "Low": 2, "Informational": 3, "Optimization": 4}.get(x["impact"], 5),
                ...
                for elem in vuln["elements"][:3]:
                    if "source_mapping" in elem:
                        ...
                        if "filename_relative" in source:
                            report += f"- {source.get('filename_relative', 'unknown')}\n"
