...
class TorProxyTool(BaseTool):
    ...
    def __init__(
        self, proxy_host: str = "127.0.0.1", proxy_port: int = 9050, timeout: int = 30
    ):
        super().__init__()
    ...
    def test_connection(self) -> Dict[str, Any]:
        try:
            ...
            response = session.get(
                "https://check.torproject.org/api/ip", timeout=self.timeout
            )
            ...
            if is_tor:
                return self._success_response(
                    {"connected": True, "is_tor": True, "exit_ip": ip},
                    f"Successfully connected to Tor network (Exit IP: {ip})",
                )
            else:
                return self._error_response(
                    "Connected but not using Tor network", f"Current IP: {ip}"
                )
            ...
            return self._error_response(
                f"Unexpected error: {str(e)}", "Failed to test Tor connection"
            )
    ...
    def get_exit_ip(self) -> Dict[str, Any]:
        try:
            ...
            return self._success_response(
                {"exit_ip": exit_ip, "provider": "ipify"}, f"Tor exit IP: {exit_ip}"
            )
        ...
        try:
            ...
            if parse_json:
                try:
                    ...
                    return self._error_response(
                        "Response is not valid JSON", f"URL: {url}"
                    )
            else:
            ...
            return self._error_response("Failed to connect via Tor", f"URL: {url}")
        except requests.exceptions.Timeout:
            return self._error_response(
