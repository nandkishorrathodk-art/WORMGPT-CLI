...
Generate the complete Python code:"""
        ...
        try:
            response = llm_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert Python developer. Generate only raw Python code without any markdown formatting or explanations."},
            ...
            return self._error_response(
                f"Tool generation failed: {str(e)}",
                details=f"Tool name: {tool_name}"
            )
        ...
        try:
            ...
            for name in dir(module):
                ...
                if (isinstance(obj, type) and 
                    ...
                    any(base.__name__ == 'BaseTool' for base in obj.__bases__)):
    ...
    def _analyze_code(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis_goal = parameters.get("analysis_goal", "Analyze this code and suggest improvements")
...
Analysis:"""
    ...
    def _modify_code(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        error = self._validate_parameters(parameters, ["file_path", "modification_goal"])
        if error:
            return self._error_response(error)
...
Modified code:"""
        ...
        try:
            response = llm_client.chat.completions.create(
                model=llm_client.default_model,
                messages=[
                    {"role": "system", "content": "You are an expert Python developer. Generate only raw Python code without any markdown formatting."},
            ...
            if require_approval:
                ...
                try:
                    ...
                    return self._error_response(f"Failed to write modified code: {str(e)}")
    ...
    def _ensure_proper_imports(self, code: str) -> str:
        if 'from .base_tool import BaseTool' not in code and 'from base_tool import BaseTool' not in code:
