...
class OPSECDrone(BaseDrone):
        ...
        if result["success"]:
            ...
            if data.get("is_tor"):
                return self._success_response(
                    data,
                    f"Tor connection verified (Exit IP: {data.get('exit_ip')})"
                )
            else:
                return self._error_response(
                    "Not using Tor network",
                    f"Current IP: {data.get('exit_ip')}"
                )
        else:
            return result
    ...
    def _get_tor_ip(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["success"]:
            ...
            return self._success_response(
                data,
                f"Tor exit IP: {data.get('exit_ip')}"
            )
        else:
            return result
    ...
    def _fetch_url_via_tor(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["success"]:
            ...
            return self._success_response(
                data,
                f"Fetched {url} via Tor ({data.get('content_length')} bytes)"
            )
        else:
            return result
