...
class HumanFeedbackDialog(ModalScreen[str]):
    ...
    def compose(self) -> ComposeResult:
        with Container(classes="dialog"):
            yield Label("🐝 QUEEN REQUIRES YOUR GUIDANCE", classes="dialog-title")
...
class WormGPTHiveTUI(App):
    ...
    def initialize_hive(self) -> None:
        ...
        try:
            ...
            mission_log.write("[green]✓ Queen ready[/green]\n")
    ...
    def update_drone_status(self) -> None:
        ...
        for drone_name, drone_info in capabilities["drones"].items():
            table.add_row(
                drone_name, "[green]READY[/green]" if drone_info else "[dim]IDLE[/dim]"
            )
    ...
    async def execute_mission(self) -> None:
        ...
        mission_log.write("[bold green]🐝 QUEEN: Initiating new mission[/bold green]")
        mission_log.write(f"[bold]GOAL:[/bold] {goal}")
        mission_log.write(f"[bold cyan]{'='*60}[/bold cyan]\n")

        self.current_worker = self.run_worker(self.run_mission(goal), exclusive=True)
    ...
    async def run_mission(self, goal: str) -> None:
        ...
        try:
            modified_queen = ModifiedQueenOrchestrator(
                self.queen, self.log_mission_update, self.request_human_feedback_async
            )

            result = await asyncio.to_thread(
                modified_queen.execute_mission, goal, verbose=False
            )
    ...
    def log_mission_update(self, update_type: str, data: Dict[str, Any]) -> None:
        ...
        if update_type == "planning":
            mission_log.write("[cyan]📋 QUEEN: Planning mission...[/cyan]")

        elif update_type == "plan_created":
            steps = data.get("steps", 0)
            mission_log.write(
                f"[cyan]📋 QUEEN: Mission plan created with {steps} steps[/cyan]\n"
            )
    ...
    def show_capabilities(self) -> None:
        ...
        for drone_name, drone_info in capabilities["drones"].items():
            table.add_row("Drone", drone_name, drone_info.get("description", "")[:40])
            ...
            if step.status == "failed":
                ...
                if isinstance(reflection, dict):
                    ...
                    if reflection.get("next_action") == "request_human_feedback":
                        ...
                        if revised_plan:
                            remaining_steps = self.queen._parse_plan_from_llm(
                                revised_plan
                            )
