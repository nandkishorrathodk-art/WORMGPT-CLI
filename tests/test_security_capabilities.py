...
class TestSecurityAnalyzerTool:
    ...
    def security_tool(self):
        with patch.object(SecurityAnalyzerTool, '_verify_slither_installation', return_value=True):
...
class TestSecurityDrone:
    ...
    @patch('subprocess.run')
    def test_analyze_contract_file(self, mock_run, security_drone, sample_contract_path):
        mock_run.return_value = Mock(
            returncode=0,
    ...
    @patch('subprocess.run')
    def test_full_security_audit(self, mock_run, security_drone, sample_contract_path, tmp_path):
        mock_run.return_value = Mock(
            returncode=0,
    ...
    def test_poc_generation_for_different_vulnerabilities(self, security_drone, tmp_path):
        vulnerabilities = [
